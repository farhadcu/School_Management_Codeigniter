...
class Admin extends CI_Controller {
	...
	function result_process($param1 = '', $param2 = '', $param3 = '') 
	{
		...
		if ($param1 == 'process') 
		{
			...
			$this->load->model('result_process_model','rp');
			$this->rp->set_param($class_id);
			$progress1=$this->rp->verify_marks();
			$progress2=$this->rp->update_subject_result();
			$progress3=$this->rp->update_exam_result();
			$progress4=$this->rp->getrate_merit_list();
			return;
        }
	...
	function sif_form($param1 = '', $param2 = '', $param3 = '')
	{
		$page_data['page_name'] = 'sif_form';
        $page_data['page_title'] = translate('SIF Form');
        $this->load->view('index', $page_data);
	}
...
//        $page_data['reports_by_date'] = $this->db->get_where('income',array('start_date <='=>$page_data['start_date'],'end_date >='=>$page_data['end_date']))->result_array();
    ...
    function backup_restore($operation = '', $type = '') {
        ...
        if ($operation == 'restore') {
            ...
            $this->session->set_flashdata('flash_message', 'Backup Restored');
